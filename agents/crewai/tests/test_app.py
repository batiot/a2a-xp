"""Tests for the CrewAI A2A agent application.
...
CrewAI's Crew.kickoff() is mocked so tests run without a real API key.
"""
...
# ---------------------------------------------------------------------------
# AgentCard discovery tests (task 4.6)
# ---------------------------------------------------------------------------
...
def test_agent_card_name(client: TestClient) -> None:
    """AgentCard name identifies this as the CrewAI agent."""
    resp = client.get("/.well-known/agent.json")
    assert "CrewAI" in resp.json()["name"]


# ---------------------------------------------------------------------------
# message/send tests (task 4.6)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AgentExecutor unit test — get_user_input and asyncio.get_running_loop
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_executor_uses_get_user_input_and_running_loop() -> None:
    """RhymeCompleterExecutor calls context.get_user_input() and uses get_running_loop."""
    # Mock the crewai module and build_crew before importing the executor
    mock_crew_module = MagicMock()
    mock_crew = MagicMock()
    mock_crew.kickoff.return_value = "chapeau de paille"
    mock_crew_module.build_crew.return_value = mock_crew

    with patch.dict("sys.modules", {"app.crew": mock_crew_module, "crewai": MagicMock()}):
        ...
        with patch("app.agent_executor.build_crew", return_value=mock_crew):
